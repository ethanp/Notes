...
* A Spark __job__ is a series of transformations and an action
* It will read data, transform it, and put it somewhere
* This _job_ is logically a DAG of of the different transformations and actions
* This DAG will have sets of transformations that can be pipelined to occur
  together
* A set of transformations that the "DAG scheduler" has noticed can be
  pipelined and run all within one _executor_ (JVM) is called a __stage__
* A stage boundary would occur because data must exit this executor, either
  because of a _shuffle_, or to _output_ the data
* The stage will (hopefully) occur in _multiple_ executors in parallel, with
  each executor processesing a different _partition_ of the data
* A _particular_ stage in a particular partition is called a __task__
* This is the unit that must be scheduled by the DAG scheduler
* For instance, the scheduler might send such a task to _Mesos_, who will
  figure out which executor to send it to (see more on Mesos below)
* A task has serialized functions inside it, which the executor will
  deserialize and execute upon the task's data
